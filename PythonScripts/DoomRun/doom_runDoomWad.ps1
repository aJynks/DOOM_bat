...
$pak = @{
    "pak1" = @(
        "d:\Projects\DoomProjects\_SourcePorts\_Wads and Mods\_tweaks\NyanDoom\aJynksed-exHUD.wad",
        "d:\Projects\DoomProjects\_SourcePorts\_paks\statusBar-GFXonly-Glasses.wad"
    )
    "pak2" = @(
        "d:\Projects\DoomProjects\_SourcePorts\_Wads and Mods\_tweaks\NyanDoom\kat-exHUD.wad",
...
function Show-Help {
    ...
    Write-Host "  doom [PORT] [IWAD] [PAK...] [OPTIONS...] [--add PAK...]" -ForegroundColor White
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "  --add <pak> loads paks at the VERY END of the -file list instead." -ForegroundColor White
    Write-Host "  You can chain multiple pak names after --add." -ForegroundColor White
    Write-Host ""
    Write-Host "  Example:" -ForegroundColor Gray
    Write-Host "    doom pak1 --add pak2" -ForegroundColor Gray
    Write-Host "  Becomes:" -ForegroundColor Gray
    Write-Host "    -file <pak1 wads...> <auto/project wad> <pak2 wads...>" -ForegroundColor Gray
    Write-Host ""
    Write-Host "  doom --listpaks    (prints all defined pak names and exits)" -ForegroundColor White
    ...
    Write-Host "  - --skip : omit the release WAD entirely (ignored outside project folders)" -ForegroundColor White
    Write-Host "             Useful when passing a custom -file directly:" -ForegroundColor Gray
    Write-Host "             doom doom -nosound -warp 1 1 -file build\editor-assets.wad --skip" -ForegroundColor Gray
...
#-------------------------------------------------------------------------------------------------
# --listpaks switch (early exit)

$listPaksTokens = @("--listpaks", "-listpaks", "/listpaks")
$wantsListPaks = $false
foreach ($a in $command_raw) {
    if ($listPaksTokens -icontains $a) { $wantsListPaks = $true; break }
}
if ($wantsListPaks) {
    Write-Host ""
    Write-Host ("== Defined Paks ==") -ForegroundColor Yellow
    if ($pak.Count -eq 0) {
        Write-Host "  (none defined)" -ForegroundColor Gray
    } else {
        foreach ($key in ($pak.Keys | Sort-Object)) {
            $wadCount = $pak[$key].Count
            Write-Host ("  " + $key + "  (" + $wadCount + " WAD" + $(if ($wadCount -ne 1) { "s" } else { "" }) + ")") -ForegroundColor Cyan
        }
    }
    Write-Host ""
    exit
...
#-------------------------------------------------------------------------------------------------
# --skip flag: strip from raw args unconditionally; only meaningful in DoomMake mode

$skipRelease = $false
$command_raw_clean = @()
foreach ($a in $command_raw) {
    if ($null -ne $a -and $a.ToString() -ine "--skip") {
        $command_raw_clean += $a
    } else {
        $skipRelease = $true
    }
}
$command_raw = $command_raw_clean
...
if ($hasDoomMakeProject) {
    ...
    $iwadExplicitlySet = $false

    foreach ($arg in $command_raw) {

        # --add switch: everything after this that is a pak name goes to the tail list
        if ($arg -ieq "--add") { $inAddMode = $true; continue }
        ...
        if ($pak.ContainsKey($arg)) {
            ...
            if ($inAddMode) {
                foreach ($p in $pak[$arg]) { $addPakWads += $p }
            } else {
                $usedPakNames += $arg
        ...
        $filteredArgs += $arg
    } # override defaults with conf values, unless user explicitly set them on the CLI
    if (-not $iwadExplicitlySet -and -not [string]::IsNullOrWhiteSpace($loader.iwad)) {
    ...
    for ($i = 0; $i -lt $addPakWads.Count; $i++) {
        Validate-Path $addPakWads[$i] "Add-Pak WAD" ("add#" + ($i + 1))
    }
    ...
    if (-not $skipRelease) {
        ...
        # release is REQUIRED (unless --skip was passed)
    ...
    # Build PWAD merge/file list: pak(s) first, then release (unless --skip), then --add pak(s) last.
    # The .deh patch is NOT put in this list; it is appended as -deh after the PWADs.
    $fileList = @()
    if ($pakWads.Count -gt 0) { $fileList += $pakWads }
    if (-not $skipRelease) { $fileList += $releasePath }
...
# Launch
if ($pakWads.Count -gt 0 -or $null -ne $wadFullPath -or $addPakWads.Count -gt 0) {
    $fileList = @()
    if ($pakWads.Count -gt 0) { $fileList += $pakWads }
