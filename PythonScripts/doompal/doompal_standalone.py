...
def _ansi_colors():
    """
    Return ANSI colour codes for help output, or empty strings when the
    output is piped/redirected or the console can't handle them.
    On Windows 10+ the empty os.system('') call switches the console
    into VT processing mode so ANSI codes work.
    """
    if not sys.stdout.isatty():
        return {k: "" for k in ("TI", "HD", "CM", "FL", "DM", "R")}
    if os.name == "nt":
        os.system("")  # enable ANSI/VT processing on Windows 10+ consoles
    return {
        "TI": "\033[1;96m",  # title      - bold bright cyan
        "HD": "\033[1;93m",  # headers    - bold bright yellow
        "CM": "\033[92m",    # commands   - bright green
        "FL": "\033[96m",    # flags/args - bright cyan
        "DM": "\033[90m",    # dim notes  - grey
        "R":  "\033[0m",     # reset
    }


def print_compact_help():
    """Print compact command listing (coloured)"""
    c = _ansi_colors()
    TI, HD, CM, FL, DM, R = c["TI"], c["HD"], c["CM"], c["FL"], c["DM"], c["R"]

    print(f"{TI}doompal{R} - Doom palette and colormap tool {DM}v{__version__}{R}")
    print()
    print(f"{HD}USAGE:{R}")
    print(f"  doompal {FL}<input>{R}                                    Auto-batch mode")
    ...
    print("              identical/simular (cleared pixels, original positions,")
    print("              transparent bg) and aligned (kept colours packed from")
    print("              slot 0, transparent tail). Never auto-batched.")
    print(f"              {FL}--rgb{R} : Weighted RGB metric (default tolerance 1.1)")
    ...
    print()
    print(f"{HD}OPTIONS:{R}")
    print(f"  {FL}-h, --help{R}           Show this help")
    print(f"  {FL}--version{R}            Show version")
    print()
    print(f"{HD}NOTES:{R}")
    ...
    print(f"      {DM}Auto-batch: cube + playpal + colormap + split from the WAD's PLAYPAL{R}")
    print(f"  {CM}doompal cube{R} pal0.png")
    print(f"      {DM}Make a .cube LUT for image editors{R}")
    print(f"  {CM}doompal playpal{R} pal0.pal playpal.png {FL}--blank{R}")
    ...
    print(f"      {DM}Duplicate/similar colour analysis (CIE Lab, tolerance 2.0){R}")
    print(f"  {CM}doompal cleanpal{R} doom2.wad {FL}--tolerance 2.3 --layers{R}")
    print(f"      {DM}Analyse the WAD's PLAYPAL, plus one PNG per duplicate group{R}")
    print(f"  {CM}doompal cleanpal{R} pal0.png {FL}--rgb{R}")
    print(f"      {DM}Legacy weighted-RGB metric (tolerance 1.1){R}")
...
CLEANPAL_DEFAULT_TOL_LAB = "2.0"
CLEANPAL_DEFAULT_TOL_RGB = "1.1"
...
class _CleanpalUF:
    """Union-find (disjoint sets) for transitive similarity clustering."""
...
def _cleanpal_geometry(path):
    """
    Work out output canvas + paint geometry for cleanpal, based on the
    input file type.

    Returns (label, canvas_wh, paint_wh, paint_xy_fn).
      SLADE grid PNG -> input size, blocks of the input's cell size
      strip PNGs     -> 256 x rows, full-height 1px columns
      .pal / .wad    -> 256 x 1, single pixels
    """
    ext = Path(path).suffix.lower()
    if ext == ".wad":
        return ("WAD PLAYPAL", (COLORS, 1), (1, 1), lambda s: (s, 0))
    ...
    if kind == "sladeStylePNG":
        ...
        return (
            f"SLADE grid ({w}x{h}, {cw}x{ch} cells)",
            (w, h),
            (cw, ch),
            lambda s: ((s % GRID_SIZE) * cw, (s // GRID_SIZE) * ch),
        )

    if kind in ("playpal14PNG", "playpal1PNG", "colormap34PNG"):
        with Image.open(path) as im:
            w, h = im.size
        rows = h // (w // COLORS)  # normalise scaled strips
        return (
            f"strip PNG (256x{rows}, row 0 = pal0)",
            (COLORS, rows),
            (1, rows),
            lambda s: (s, 0),
        )

    if kind in ("playpal.pal binary", "pal0.pal Binary"):
        return ("binary pal0", (COLORS, 1), (1, 1), lambda s: (s, 0))

    raise ValueError(f"Unsupported input for cleanpal: {path}")


def _cleanpal_paint(img, paint_xy, paint_wh, slot, rgb):
    """Paint one palette entry onto an RGBA canvas."""
    x0, y0 = paint_xy(slot)
    pw, ph = paint_wh
    px = img.load()
    for dy in range(ph):
...
def cmd_cleanpal(args):
    ...
    Outputs (next to the input):
      ...
      name-simular-(TYPE-tol).png    cleared similar pixels, same treatment
      name-aligned-(TYPE-tol).png    kept colours packed from slot 0,
                                     tail transparent
      --layers adds one PNG per set/cluster: ...-keepNNN.png
    """
    input_path = args.input
    ...
    tol_str = args.tolerance if args.tolerance is not None else (
        CLEANPAL_DEFAULT_TOL_LAB if use_lab else CLEANPAL_DEFAULT_TOL_RGB
    )
    try:
        tol = float(tol_str)
        if tol < 0:
        ...
        print(f"ERROR: Invalid tolerance value: {tol_str}", file=sys.stderr)
        return 1

    mtype = "CIE" if use_lab else "RGB"
    token = f"({mtype}-{tol_str})"
    ...
    labs = [_srgb_to_lab(r, g, b) for (r, g, b) in palette]

    def dist(a, b):
        if use_lab:
            dl = labs[a][0] - labs[b][0]
            da = labs[a][1] - labs[b][1]
            db = labs[a][2] - labs[b][2]
            return (dl * dl + da * da + db * db) ** 0.5
        dr = abs(palette[a][0] - palette[b][0])
        dg = abs(palette[a][1] - palette[b][1])
        db = abs(palette[a][2] - palette[b][2])
        return ((0.299 * dr) ** 2 + (0.587 * dg) ** 2 + (0.114 * db) ** 2) ** 0.5

    # ---- IDENTICAL (exact RGB) ----
    exact_map = {}
    for idx, rgb in enumerate(palette):
        exact_map.setdefault(rgb, []).append(idx)
    ...
    # ---- identical / simular / aligned ----
    p = Path(f"{base}-identical-{token}.png")
    paint_groups(identical_groups).save(p)
    written.append(p)

    p = Path(f"{base}-simular-{token}.png")
    paint_groups(similar_groups).save(p)
    written.append(p)

    img = new_canvas()
    for slot, idx in enumerate(kept):
        _cleanpal_paint(img, paint_xy, paint_wh, slot, palette[idx])
    p = Path(f"{base}-aligned-{token}.png")
    img.save(p)
    written.append(p)

    # ---- per-group layer files ----
    if args.layers:
        for name, groups in (
    ...
    # ---- summary ----
    print(f"Input : {input_path}  [{label}]")
    print(
    ...
    print(f"Kept: {len(kept)}   Cleared: {len(cleared)}")
...
def main():
    ...
    try:
        ...
        elif command == 'cleanpal':
            return cmd_cleanpal(args)
        else:
