...
:: Path and config file variables
...
:: Create subdirectories for Demos, Screenshots, and Saves if they don’t exist
for %%D in (Demos Screenshots Saves) do (
    if not exist "%configDir%\%%D" (
        echo Creating subdirectory: %%D
        mkdir "%configDir%\%%D"
    )
)
