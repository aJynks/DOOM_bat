...
def check_dependencies():
    m=[]
    try:import PIL
    except:m.append("Pillow")
    try:import numpy
    except:m.append("numpy")
    if m:
        print("ERROR: Missing required dependencies!",file=sys.stderr)
        print("",file=sys.stderr)
        print("Missing packages:",", ".join(m),file=sys.stderr)
        print("",file=sys.stderr)
        print("To install, run:",file=sys.stderr)
        print(f"  pip install {' '.join(m)}",file=sys.stderr)
        print("",file=sys.stderr)
        print("Or install all:",file=sys.stderr)
...
def main():
    if len(sys.argv)<2 or sys.argv[1]in['-h','--help','-help']:print_compact_help();return 0
    c=sys.argv[1]
    if c not in['cube','batch','playpal','colormap','colourmap','blank','slade','extract','split']:
        c='batch'
        f=Path(sys.argv[1]).name.rsplit(".",1)[0]
        a=[sys.argv[1],f]+sys.argv[2:]
    else:a=[x for x in sys.argv[2:]if not x.startswith('-')]
    g=Args(input=Path(a[0])if a else None,output=Path(a[1])if len(a)>1 else None,
           slade='--slade'in sys.argv or'-slade'in sys.argv,blank='--blank'in sys.argv or'-blank'in sys.argv,
           all='--all'in sys.argv or'-all'in sys.argv,playpal='--playpal'in sys.argv or'-playpal'in sys.argv,
           boom='--boom'in sys.argv or'-boom'in sys.argv,boom_list='--boomlist'in sys.argv or'-boomlist'in sys.argv,cell=8)
    try:return{'cube':cmd_cube,'batch':cmd_batch,'playpal':cmd_playpal,'colormap':cmd_colormap,
               'colourmap':cmd_colormap,'blank':cmd_blank,'slade':cmd_slade,
               'extract':cmd_extract,'split':cmd_split}[c](g)
    except Exception as e:
        print(f"ERROR: {e}",file=sys.stderr);import traceback;traceback.print_exc();return 1
