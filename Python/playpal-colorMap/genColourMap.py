...
def build_colormap(palette):
    ...
    for l in range(NUMLIGHTS):
        ...
        for r, g, b in palette:
            nr = (r * (NUMLIGHTS - l) + NUMLIGHTS // 2) // NUMLIGHTS
            ng = (g * (NUMLIGHTS - l) + NUMLIGHTS // 2) // NUMLIGHTS
            nb = (b * (NUMLIGHTS - l) + NUMLIGHTS // 2) // NUMLIGHTS
...
def main():
    if len(sys.argv) != 3:
        print("Usage: py generateColourMap.py inputpalette output.png")
        return 1
    ...
    print("Generated colormap PNG:", outp)
    print("Rows:", len(colormap), "(includes invulnerability + black)")
