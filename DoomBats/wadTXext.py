...
if missing_deps:
    ...
    print(f"python -m pip install --upgrade pip && python -m pip install {' '.join(missing_deps)}")
    sys.exit(1)
    ...
    with WadReader(args.wadfile) as wad:
        ...
        with WadReader(args.iwad) as iwad:
            ...
            if args.list:
                ...
                # Create singlePatch subdirectory
                single_patch_dir = composite_dir / 'singlePatch'
                single_patch_dir.mkdir(exist_ok=True)
                ...
                if args.png:
                    ...
                    for texture_name in textures_used:
                        ...
                        # Handle single-patch textures
                        if len(patches) == 1:
                            # Copy to composite/singlePatch/ with texture name
                            if from_iwad:
                                iwad_single_dir = single_patch_dir / iwad_name
                                iwad_single_dir.mkdir(exist_ok=True)
                                output_dir = iwad_single_dir
                            else:
                                output_dir = single_patch_dir
                            
                            # Get the patch
                            patch = texture_def.patches[0]
                            if patch.patch_num < len(pnames):
                                pname = pnames[patch.patch_num]
                                
                                # Try PWAD first, then IWAD
                                ...
                                if lump:
                                    ...
                                    if lump:
                                        ...
                                        print(f"  Warning: Patch {pname} not found for texture {texture_name}")
                                        continue
                                ...
                                if graphic.pixels:
                                    ...
                                    save_as_png(rgba_data, graphic.width, graphic.height, output_file)
                                    print(f"  {texture_name} (1 patch) -> {output_file}")
                            continue
                        
                        # Handle multi-patch textures
                        if from_iwad:
